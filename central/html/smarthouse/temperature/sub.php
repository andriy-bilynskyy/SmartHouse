<?php
    ...
    {
        $result = mysqli_query($db, "SELECT address FROM subscribed_temperature_sensors");
        while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
        {
            $addr = $row["address"];
            $desc = mysqli_real_escape_string($db,$_REQUEST[$row["address"]]);
            if(mysqli_query($db,"UPDATE subscribed_temperature_sensors SET description='$desc' WHERE address='$addr'"))
            {
                if(mysqli_affected_rows($db) > 0)
                {
                    mysqli_query($db,"INSERT INTO data_log (unit, message) VALUES ('TEMPERATURE', 'Updated sensor $addr description - $desc')");
                }
            }
        }
    }
...
<html> 
    ...
    <body bgcolor = "#FFFFFF">
        <div style = "background-color:#333333; color:#FFFFFF; padding:3px;"><b>Subscribed temperature sensors</b></div><br/>
        ...
        <form action = "" method = "post">
            ...
            <table cellpadding=5px><col style="width:15ch"><col style="width:100%"><tr><th>address</th><th>description</th></tr>
                <?php
                    $result = mysqli_query($db,"SELECT * FROM subscribed_temperature_sensors");
                    while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
                    {
                        echo "<tr>";
